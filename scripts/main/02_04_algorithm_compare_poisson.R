...
# Runs simulations using Poisson distribution.
# ------------------------------------------------------------------------------
...
cores_prop <- 0.85; n_cores <- round(cores_prop*availableCores())
...
df_psi <- foreach(i = 1:n_compare, .combine = "rbind", 
  ...
  test_dt <- data.frame(z = 1:100, 
                        e = rnorm(100)) %>% 
    mutate(value = rpois(100, exp(1 + b1*z + b2*(z - psi_true)*(z > psi_true))))
  ...
  seg_fit <- segmented(glm(value ~ z, data = test_dt, family = "poisson"), 
                       seg.Z = ~z, 
                       npsi = 1, 
