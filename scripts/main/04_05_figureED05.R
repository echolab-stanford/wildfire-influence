...
# ------------------------------------------------------------------------------
# Figure ED5: trend-change classes sensitivity ----
plot_grid(
  # Panel a
  mean_classified %>% 
    filter(!(state_abbr == "US" & grepl("region", sample_name))) %>% 
    dplyr::select(state_name, sample_factor, change_group_factor) %>% 
    {ggplot(data = ., 
            aes(y = state_name,
                x = sample_factor, 
  ...
  mean_classified %>% 
    ...
    {ggplot(data = ., 
        ...
        geom_text(data = mean_classified %>% 
                    filter(state_abbr != "US") %>% 
                    summarise(n = n(), .by = c(sample_factor, change_group_factor)) %>% 
                    arrange(sample_factor, desc(change_group_factor)) %>% 
                    mutate(lower = cumsum(replace_na(lag(n), 0)),
                           upper = cumsum(n),
                           mid = (lower + upper)/2,
                           .by = sample_factor) %>% 
                    filter(change_group_factor %in% 
                             c("acceleration", "no sig. early decline") == FALSE), 
                  aes(x = mid, y = sample_factor, label = n, 
                      color = I(ifelse(change_group_factor == "reversal", "grey85", "grey15"))), 
                  inherit.aes = F) + 
        scale_y_discrete(limits = rev) +
  ...
  labels = "auto") %>% 
  ggsave(file.path(path_github, "figures", "raw", "figureED05_pmtrend_sensitivity.pdf"), ., 
         width = 8, height = 7)
