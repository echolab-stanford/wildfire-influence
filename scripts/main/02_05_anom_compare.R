...
# Conducts estimation using anomalies bottom-coded to 0.
# ------------------------------------------------------------------------------
...
cores_prop = 0.45; n_cores <- round(cores_prop*availableCores())
...
if (!conus) {stop("Only CONUS is estimation is supported for anomaly comparison script.")}

# Build objects over which you're going to loop to compare (overwriting globals 
# where appropriate)
sample_names <- c("pos_anom")
file_names <- file.path(
  path_dropbox, "output", 
  paste0(ifelse(conus, "other/anom_compare_conus_", 
                "other/anom_compare_regional_"), 
         pm_type, "_", n_boot, "b", 
         "_alpha", break_alpha*100,  "_corner", corner_push*100,  
         "_fitstat", break_fitstat, ifelse(smart_init, "_smartinit", "_randominit"), 
...
for (j in 1:length(sample_names)) {
  ...
  if (!file.exists(file_names[j]) | overwrite) {
    ...
    list_sample <- future_Map(
      ...
      MoreArgs = trend_args, 
      future.seed = 1234, 
      # future.scheduling and future.chunk.size arguments can be set to force 
      # single future for each element in X
...
## Results check
apply(df_sample, 2, function(x) {sum(is.na(x))})
df_plot <- df_sample %>% filter(!is.na(break_year))
ggplot(df_plot, aes(x = break_year)) + 
