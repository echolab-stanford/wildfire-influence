...
# ------------------------------------------------------------------------------
# Figure 1: regional data viz ---- 
station_year %>% 
  filter(totalPM_n > 50) %>% 
  filter(n() >= 15, .by = id) %>% 
  rbind(., 
        mutate(., climate_regions = "Contiguous US")) %>% 
  summarise(across(c(totalPM_mean, totalPM_extremePct), mean), 
            .by = c(climate_regions, year)) %>% 
  # Add rows for the break year so we can get the joint plotted correctly for predictions
  rbind(., 
        filter(., year == 2000) %>% 
          mutate(year = mean_break, 
                 totalPM_mean = NA,
                 totalPM_extremePct = NA), 
        filter(., year == 2000) %>% 
          mutate(year = extreme_break, 
                 totalPM_mean = NA,
                 totalPM_extremePct = NA)) %>% 
  {mutate(., 
          pred_mean = predict(feols(
            totalPM_mean ~ climate_regions:year + climate_regions:years_post | climate_regions, 
            data = mutate(., years_post = (year - mean_break)*(year > mean_break)), 
            lean = F), 
            newdata = mutate(., years_post = (year - mean_break)*(year > mean_break)) %>% 
              dplyr::select(-totalPM_mean, - totalPM_extremePct)))} %>% 
  {ggplot(data = filter(., !is.na(totalPM_mean)), 
          aes(x = year, y = totalPM_mean)) + 
      geom_line() + 
      geom_line(data = .,
                aes(y = pred_mean), color = "red") +
      geom_vline(xintercept = mean_break, 
                 color = "red", 
                 linetype = "dashed") + 
      facet_wrap(~climate_regions, 
                 scales = "free") + 
      theme_classic() + 
      theme(strip.background = element_blank())} %>% 
  ggsave(filename = file.path(path_github, "figures", "raw", "figure01_trends.pdf"), 
         width = 11, height = 7)
