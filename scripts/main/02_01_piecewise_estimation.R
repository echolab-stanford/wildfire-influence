...
# ------------------------------------------------------------------------------
#### Estimation ####
# Use this if you want to randomize start
# Useful if running on multiple machines
j_order <- sample(1:length(sample_names), length(sample_names))
for (j in j_order) {
  ...
  if (!file.exists(file_names[j]) | overwrite) { 
    ...
    list_boot <- future_Map(
      ...
      seed = 1:n_boot, 
      MoreArgs = boot_args, 
      # Turn off seed because bootstrap function sets a seed directly
      future.seed = NULL
    )
    list_sample <- future_Map(
      ...
      MoreArgs = trend_args, 
      # We need future.seed = T here because we're doing bootstrap 
